...
### Task 15: Register all tools with FastMCP server
...
### Task 16: Register MCP server with Claude Code
...
### Task 17: Update AGENTS.md install policy

**Files:**
- Modify: `~/.config/opencode/AGENTS.md`

**Step 1: Update the Installation and Update Policy section**

Replace the current `## Installation and Update Policy (Bazzite)` section with the corrected 6-tier hierarchy based on official docs.bazzite.gg:

```markdown
## Installation and Update Policy (Bazzite)
- Goal: stay current (`aggressive latest`) while preserving host stability.
- Preferred install order (official Bazzite 6-tier hierarchy from docs.bazzite.gg):
  1. `ujust` first — check `ujust --summary` for setup/install/configure commands.
  2. `flatpak` for GUI apps (Flathub preferred). Primary installation method on Bazzite.
  3. `brew` for CLI/TUI tools only. No GUI apps via brew.
  4. `distrobox` for mutable dev environments, other distro package managers (apt, pacman, etc.); `quadlet` for persistent containerized services.
  5. `AppImage` from trusted sources only. Manage via Gear Lever.
  6. `rpm-ostree` absolutely last resort. Can freeze updates, block rebasing, cause dependency conflicts. Avoid if any alternative exists.
- Host rule: avoid `dnf install` directly on the immutable host.
- rpm-ostree warning: layered packages can pause system updates, prevent rebasing, and cause dependency conflicts. Keep layered packages at absolute zero if possible.
- Website installers / `curl` scripts: last resort; prefer package managers, inspect script first, prefer user-space installs.
- Version pinning: no pinning by default.
- Updates: prioritize latest stable releases. Use `ujust update` for full system update.
- Unattended updates are allowed for routine upgrades (for example `brew upgrade`, standard Flatpak updates, and normal system update flows).
- Ask first before non-routine or potentially disruptive system operations (for example rebases, package removals that may affect workflows, major host-layering changes, or rollback execution).
- Do NOT rebase to switch desktop environments — backup and reinstall instead.
- Hostname must be ≤20 characters (longer breaks Distrobox).
- Rollback: keep a rollback path documented before major host changes; use `rpm-ostree rollback` when needed.
```

**Step 2: Commit** (in bazzite-mcp repo, note the change for reference)

No git commit needed — AGENTS.md is outside the repo. Just apply the edit.
