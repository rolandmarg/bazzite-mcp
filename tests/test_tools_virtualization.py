...
@patch("bazzite_mcp.tools.virtualization.preflight.run_command")
@patch("bazzite_mcp.tools.virtualization.lifecycle.run_command")
@patch("bazzite_mcp.tools.virtualization.lifecycle.run_audited")
def test_manage_vm_prepare_atomic(
    mock_audited: MagicMock,
    mock_command: MagicMock,
    mock_preflight_command: MagicMock,
    tmp_path: Path,
) -> None:
    mock_audited.return_value = _ok("ok")
    mock_command.side_effect = _preflight_ok_command
    mock_preflight_command.side_effect = _preflight_ok_command

    with patch(
        "bazzite_mcp.tools.virtualization.state.VM_OPERATION_STATE_FILE",
...
def test_manage_vm_prepare_reboot_required(
    ...
    mock_preflight_command.side_effect = _side_effect
...
@patch("bazzite_mcp.tools.virtualization.lifecycle._vm_prepare")
...
@patch("bazzite_mcp.tools.virtualization.preflight.run_command")
@patch("bazzite_mcp.tools.virtualization.lifecycle.run_audited")
def test_manage_vm_prepare_raises_on_error(
    mock_run: MagicMock,
    mock_preflight: MagicMock,
    tmp_path: Path,
) -> None:
    mock_run.return_value = MagicMock(returncode=1, stdout="", stderr="failed")
    mock_preflight.side_effect = _preflight_ok_command
...
@patch("bazzite_mcp.tools.virtualization.lifecycle._host_resources")
@patch("bazzite_mcp.tools.virtualization.preflight.run_command")
@patch("bazzite_mcp.tools.virtualization.lifecycle.run_audited")
def test_manage_vm_create_default_uses_dynamic_defaults(
    ...
    with patch("bazzite_mcp.tools.virtualization.lifecycle.VM_STORAGE_DIR", tmp_path / "vms"):
...
@patch("bazzite_mcp.tools.virtualization.state.run_audited")
def test_manage_vm_rollback(mock_audited: MagicMock, tmp_path: Path) -> None:
    ...
    with patch("bazzite_mcp.tools.virtualization.state.VM_OPERATION_STATE_FILE", state_file):
