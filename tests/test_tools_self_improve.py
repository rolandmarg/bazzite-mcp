...
def test_get_server_source_path_traversal_blocked(
    mock_repo: MagicMock, tmp_path: Path
) -> None:
...
@patch("bazzite_mcp.tools.self_improve._repo_local")
def test_get_server_source_reads_valid_file(
    mock_repo: MagicMock, tmp_path: Path
) -> None:
    mock_repo.return_value = str(tmp_path)
...
@patch(
    "bazzite_mcp.tools.self_improve._repo_slug", return_value="rolandmarg/bazzite-mcp"
)
